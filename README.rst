...
When indexing an portal content, if the portal content
...
such that instead of re-indexing object security for  each descendant via a catalog query,
it should instead call reindexObject once on the portal object and allow the experimental 
LocalRolesIndex to determine which children need to be re-indexed.
